    ...
    <div className="px-4">
      ...
      <section className="max-w-6xl mx-auto pb-20">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-gold-dark font-black mb-2">Finance Card Journal</p>
            <h2 className="text-3xl md:text-4xl font-extrabold text-navy">Today's market story, as a card</h2>
            <p className="text-navy/60 mt-3 max-w-2xl">
              New card designs tied to recent finance news, with the explanation behind each card.
            </p>
          </div>
          {profile?.is_admin && (
            <Link
              to="/admin"
              className="self-start md:self-auto bg-navy hover:bg-navy-mid text-white px-5 py-2.5 rounded-xl font-bold transition-colors"
            >
              Write a post
            </Link>
          )}
        ...
        {postsLoading ? (
          ...
          <div className="bg-white border border-navy/10 rounded-2xl p-10 text-center shadow-sm">
            <h3 className="text-xl font-bold text-navy mb-2">No finance card posts yet</h3>
            <p className="text-navy/60">
              Once an admin writes the first post, it will appear here with the card on one side and the market explanation on the other.
            </p>
          </div>
        ) : (
